function y=myistft_matlab(X,stft_win_len,stft_win_overlap,nsampl)

% MYISTFT_MATLAB Multichannel inverse short-time Fourier transform (ISTFT)
% using hann windows.
%
% y=myistft_matlab(X,nsampl)
% y=myistft_matlab(X,stft_win_len,stft_win_overlap,nsampl)
% 
% Input:
...
%
% Output:
% y: nsrc x nsampl matrix or nsrc x nsampl x nchan matrix containing the
...
if nargin<3, nsampl=stft_win_len; stft_win_len=1024; stft_win_overlap=0.5*stft_win_len; end
